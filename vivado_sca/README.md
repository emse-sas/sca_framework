...
Our test bench features two types of sensors based on various technologies.
...
- *coarse length* : coarse calibration of the initial clock delay
- *fine length* : fine calibration of the initial clock delay
- *sampling length* : output sampling range
...
We implemented a FIFO based acquisition controller, this allows to read the our sensors at an accurately fixed frequency.

In our design, the output data of each sensor is connected to the data input of the FIFO, then our FIFO communicate its read output via AXI-bus.
