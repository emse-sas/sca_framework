...
CMD_err_t CMD_get_opts(char **words, size_t len, CMD_opt_t **options) {
    ...
    for (; word_idx < len; word_idx++) {
        if (words[word_idx][0] != '-' && words[word_idx - 1][0] != '-') return CMD_ERR_FORMAT;
        else if (words[word_idx][0] != '-') continue;

        if ((options[option_idx] = malloc(sizeof(CMD_opt_t))) == NULL) return CMD_ERR_ALLOC;
...
int CMD_find_option(const CMD_opt_t **options, char label) {
    ...
    for (; options[idx] != NULL && idx < CMD_MAX_OPTIONS; idx++) {
        if (options[idx]->label != label) continue;
        return idx;
    }
    return CMD_ERR_NOT_FOUND;
}
...
int CMD_parse_line(char *line, CMD_cmd_t *cmd) {
    ...
    if ((error = CMD_get_type(words[0], &cmd->type)) != CMD_ERR_NONE) return error;
    if ((error = CMD_get_opts(words, words_count, cmd->options)) != CMD_ERR_NONE) return error;
